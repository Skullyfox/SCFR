...
export default function EPTU() {
  ...
  const handleOpenDialog = () => {
    ipcRenderer.invoke('open-gameLocation-dialog').then((res) => {
      ...
      setGameLocation(`${res}/`);
      setGameStatus(true);
      ipcRenderer.invoke('check-translation', {path: `${res}\\${eptuPathPrefix}`}).then((res) => {
  ...
  const handleInstallTranslation = () => {
    ipcRenderer.invoke('install-translation', {path: `${gameLocation}\\${eptuPathPrefix}`}).then((res) => {
      ipcRenderer.invoke('check-translation', {path: `${gameLocation}\\${eptuPathPrefix}`}).then((res) => {
        setTranslationStatus(res);
      });
  ...
  const handleUninstallTranslation = () => {
    ipcRenderer.invoke('uninstall-translation', {path: `${gameLocation}\\${eptuPathPrefix}`}).then((res) => {
      ipcRenderer.invoke('check-translation', {path: `${gameLocation}\\${eptuPathPrefix}`}).then((res) => {
  ...
  useEffect(() => {
    ipcRenderer.invoke('get-user-preferences').then((row) => {
      if(row && row.GamePathEptu !== null){
        ipcRenderer.invoke('check-translation', {path: `${row.GamePathEptu}\\${eptuPathPrefix}`}).then(async (res) => {
          setTranslationStatus(res);
          if(res) {
            await ipcRenderer.invoke('check-translationUpToDate', {path: `${row.GamePathEptu}\\${eptuPathPrefix}`}).then((res) => {
              setTranslationUpToDate(res);
            });
          }
        });
        setScanStatus(false);
        setGameLocation(row.GamePathEptu);
        setGameStatus(true);
      }
    });
    ipcRenderer.on('game-found-reply', (event, arg) => {
      ipcRenderer.invoke('check-translation', {path: `${arg.gamePath}\\${eptuPathPrefix}`}).then(async (res) => {
        setTranslationStatus(res);
        if(res) {
          await ipcRenderer.invoke('check-translationUpToDate', {path: `${arg.gamePath}\\${eptuPathPrefix}`}).then((res) => {
            setTranslationUpToDate(res);
          });
        }
        setScanStatus(false);
  ...
  return scanStatus ? <ScanLoading version="EPTU" /> : (
    <React.Fragment>
      <Head>
        <title>SCFR | Traduction EPTU</title>
      </Head>
      <div className='container p-5 w-full pt-20 flex flex-col gap-5'>
        <h1 className="text-4xl">StarCitizen FR - Traduction EPTU</h1>
